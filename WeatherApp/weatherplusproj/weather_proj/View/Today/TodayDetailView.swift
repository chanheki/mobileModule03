    ...
    var body: some View {
        if let weatherData = weatherManager.weatherData {
            ScrollView {
                VStack {
                    Chart {
                        ForEach(Array(weatherData.hourly.time.enumerated())[0...23], id: \.element) { (index, time) in
                            LineMark(
                                x: .value("Time", time),
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        LazyHStack {
                            ForEach(Array(weatherData.hourly.time.enumerated())[0...23], id: \.offset) { index, date in
                                VStack {
